...
class reduxMap extends Component {
  ...
  getEndLocation = (lat, long) => {
    ...
    this.setState({
      ...
      db.collection("users").doc(this.props.token).update({
        "details.end_location": {latitude: lat,
          longitude: long,}
        })
        .then(function() {
        ...
        Geolocation.getCurrentPosition(
            (position) => {
              console.log(JSON.stringify(position.coords))
                  ...
                  ()=> db.collection("users").doc(this.props.token).update({
                    "details.start_location": {latitude: position.coords.latitude,
                      longitude: position.coords.longitude,}
                    })
                    .then(function() {
                    console.log("Document successfully updated!");
     ...
     //        image={require('../assets/images/person-walking.png')}
      ...
      //        onDragEnd={this.tweakDestination}
        ...
        { this.state.end_location && this.state.startTrek  && <Marker.Animated
           ...
           <LinearGradient colors={['#57C693', '#377848']}
           ...
           <LinearGradient colors={['#57C693', '#377848']} style={styles.myLocationButton}>
          <Image
